...
BOT_OWNER = list({int(x) for x in ("6440363814").split()})

_MGCLIENT: AgnosticClient = AsyncIOMotorClient(MONGO_DATABASE)

_DATABASE: AgnosticDatabase = _MGCLIENT["GojoAnime"]
    ...
    async def wrapper(_, message: Message):
            ...
            try:
                ...
                if nut - out < 1.2:
                    ...
                    if USER_WC[user] == 3:
                        ...
                        await clog("Gojo", f"UserID: {user}", "SPAM")
                    if USER_WC[user] == 5:
                        ...
                        await clog("Gojo", f"UserID: {user}", "BAN")
                        return
        ...
        try:
            ...
            try:
                ...
                await clog("Gojo", e, "FAILURE", msg=message)
        ...
        if user in BOT_OWNER or user == int(cqowner):
            ...
            try:
                ...
                try:
                    ...
                    await clog("Gojo", e, "FAILURE", cq=c_q)
        else:
            if cqowner_is_ch:
                if user_valid:
                    try:
...
async def take_screen_shot(
    ...
    """take a screenshot"""
    print(
        "[[[resourcescting a frame from %s ||| Video duration => %s]]]",
        video_file,
        duration,
    )
...
async def clog(
    ...
    if cq:
        data += str(cq)
        data += "\n\n\n\n"
    await app.send_message(chat_id=DATA_LOG, text=log)
    if msg or cq:
        with open("query_data.txt", "x") as output:
            output.write(data)
        await app.send_document(DATA_LOG, "query_data.txt")
        os.remove("query_data.txt")
    if replied:
        media = replied.photo or replied.sticker or replied.animation or replied.video
        media_path = await app.download_media(media)
        await app.send_document(DATA_LOG, media_path)
    if file:
        await app.send_document(DATA_LOG, file)
    if send_as_file:
        with open("dataInQuestio.txt", "x") as text_file:
            text_file.write()
        await app.send_document(DATA_LOG, "dataInQuestio.txt")
...
example: /anime Sword Art Online"""
    ...
    try:
        ...
        await clog("Gojo", title_img, "LINK", msg=message)
...
example: /manga Sword Art Online"""
    ...
    try:
        ...
        await clog("Gojo", pic, "LINK", msg=message)
        await client.send_photo(gid, failed_pic, caption=finals_, reply_markup=buttons)
...
async def character_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await clog("Gojo", img, "LINK", msg=message)
...
example: /airing Sword Art Online"""
    ...
    try:
        ...
        await clog("Gojo", coverImg, "LINK", msg=message)
...
This material is marked 18+ and not allowed in this group""",
    ...
    try:
        ...
        await clog("Gojo", pic, "LINK", msg=cq)
        await cq.edit_message_media(
